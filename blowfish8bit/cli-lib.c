...
static bool translate_hex(char const *hex_str, uint8_t *out, size_t out_len, char const **end_ptr);
...
from_hex_or_fail(char const* hexed, size_t *num_bytes)
{
    ...
    *num_bytes = input_len / 2;
    ...
    if (!translate_hex(hexed, out_buf, *num_bytes, &end_ptr)) {
        fprintf(stderr, "ERROR: failed to parse '%s' as a hex string starting with '%c'\n",
                hexed, *end_ptr);
        free(out_buf);
        exit(EXIT_FAILURE);
    }
...
static bool
translate_hex(char const *hex_str, uint8_t *out, size_t out_len, char const **end_ptr)
{
    if (end_ptr) {
        *end_ptr = NULL;
    }
